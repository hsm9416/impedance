...
class Subscriber:
    current_value = 0  # 클래스 변수로 초기화

    @classmethod
    def callback(cls, data):
        cls.current_value = data.data  # 클래스 변수 사용
        rospy.loginfo("받은 메시지: %s", cls.current_value)
    
    @classmethod
    def listener(cls):
        rospy.init_node('listener', anonymous=True)
    
        # 구독할 토픽 이름을 '/gait_phase'로 설정하고 메시지 유형은 String으로 설정
        rospy.Subscriber("/gait_phase", String, cls.callback)
    
        rospy.spin()


def gait_cycle(current_value):
    ser = init_serial()
    toggle()
    total_duration = 2
    max_value = 100
