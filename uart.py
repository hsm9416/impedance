...
def process_data(data):
    ...
    if len(split_data) == 11:  # 데이터가 올바르게 split되었는지 확인
        ...
        try:
            ...
            split_data[3] = float(split_data[3])  # l_hip_velocity를 float로 변환
            split_data[4] = float(split_data[4])  # r_hip_velocity를 float로 변환
            split_data[5] = float(split_data[5])  # l_hip_torque를 float로 변환
            split_data[6] = float(split_data[6])  # r_hip_torque를 float로 변환
            split_data[7] = float(split_data[7])  # l_hip_targetspeed를 float로 변환
            split_data[8] = float(split_data[8])  # r_hip_targetspeed를 float로 변환
